...
export const createRestaurant = async (req, res) => {
  try {
    ...
    if (existingRestaurant) {
      return res
        .status(400)
        .json({ message: "This email is already linked to an account" });
    }
    if (!req.file) {
  ...
  try {
    ...
    if (!restaurant.isVerified) {
      return res.status(403).json({
        message: "Restaurant is under review. Please wait for admin approval.",
      });
    }
...
export async function findRestaurantByName(req, res) {
  try {
    ...
    res
      .status(200)
      .json({ message: "Restaurant found successfully", restaurant });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal Server Error" });
  }
